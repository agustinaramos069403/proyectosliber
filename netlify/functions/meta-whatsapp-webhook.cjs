...
const CHACO_REGION_DISAMBIGUATION_MESSAGE =
  'En Chaco el Dr. atiende en Resistencia y en Sáenz Peña. ¿Desde cuál ciudad consultás?';

const CHACO_AMBIGUOUS_REGION_KEYWORDS = ['chaco', 'capital chaco', 'capital del chaco', 'provincia del chaco'];
...
const MEDICAL_EMERGENCY_RESPONSE_MESSAGE =
  'Entiendo que puede ser urgente. Si es una emergencia, acudí a la guardia/urgencias más cercana o llamá al 107 ahora.';
...
const MEDICAL_EMERGENCY_GUIDANCE_CONTEXT_WINDOW_MS = 30 * 60 * 1000;
...
const CONSULT_DURATION_MESSAGE =
  'La consulta suele durar entre 30 y 45 minutos según el caso y si hace falta revisar estudios.';

const SAME_DAY_STUDIES_MESSAGE =
  'A veces se pueden coordinar test de alergia y espirometría el mismo día si el Dr. lo indica en consulta; depende de cada caso y de la preparación de cada estudio. En la evaluación te orientan.';

const CONSULTATION_ORDER_REQUIREMENT_MESSAGE =
  'Si tenés obra social, conviene traer orden de consulta y prácticas autorizadas si te las dieron. Si sos particular, podés venir sin orden.';
...
const STUDY_DURATION_MESSAGE = 'Depende del estudio: un test de alergia o espirometría suele llevar entre 20 y 40 minutos. En la evaluación te orientan según lo que indique el Dr.';
...
function messageLooksLikeSedeCityCorrection(rawText) {
  ...
  if (messageLooksLikeSubstantivePatientInquiryBeyondSedeDeclaration(rawText)) return false;
  return Boolean(findSedeFromText(rawText));
}

function messageLooksLikeSubstantivePatientInquiryBeyondSedeDeclaration(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  ...
  if (
    ...
    normalized.includes('neumonolog')
  ) {
    return true;
  }
  return false;
...
function messageMentionsAmbiguousChacoRegion(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const normalized = normalizeForMatch(rawText);
  const mentionsChaco = CHACO_AMBIGUOUS_REGION_KEYWORDS.some((keyword) =>
    normalized.includes(normalizeForMatch(keyword))
  );
  if (!mentionsChaco) return false;
  if (
    normalized.includes('resistencia') ||
    normalized.includes('resis') ||
    normalized.includes('rcia') ||
    normalized.includes('saenz pena') ||
    normalized.includes('santa maria') ||
    normalized.includes('presidencia roque')
  ) {
    return false;
  }
  return true;
...
function stateLooksLikeAwaitingChacoSedeDisambiguation(priorState) {
  if (!priorState || typeof priorState !== 'object') return false;
  if (priorState.state !== 'awaiting_chaco_sede_disambiguation') return false;
  const askedAtMs = Number(priorState.awaitingChacoSedeDisambiguationAtMs);
  return Number.isFinite(askedAtMs) && Date.now() - askedAtMs <= SEDE_SELECTION_WINDOW_MS;
}
...
function conversationRecentlyAskedChacoSedeDisambiguation(priorState) {
  ...
  const askedAtMs =
    Number(priorState.awaitingChacoSedeDisambiguationAtMs) || Number(priorState.lastBotAskedSedeCityAtMs);
  return Number.isFinite(askedAtMs) && Date.now() - askedAtMs <= SEDE_SELECTION_WINDOW_MS;
}

function messageLooksLikeChacoSedeDisambiguationAnswer(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const sedeEntry = findSedeFromText(rawText);
  if (!sedeEntry) return false;
  return sedeEntry.displayName === 'Resistencia' || sedeEntry.displayName === 'Sáenz Peña';
}

function buildAwaitingChacoSedeDisambiguationStatePatch() {
  return {
    state: 'awaiting_chaco_sede_disambiguation',
    awaitingChacoSedeDisambiguationAtMs: Date.now(),
    lastBotAskedSedeCityAtMs: Date.now(),
    ...buildClearedStudyPricingContextPatch(),
  };
}

function resolvePatientSubjectFromMessage(rawText) {
  if (!rawText || typeof rawText !== 'string') return null;
  const normalized = normalizeForMatch(rawText);
  const subjectPatterns = [
    { subject: 'child', phrases: ['para mi hijo', 'para mi hija', 'mi hijo', 'mi hija', 'mi bebe', 'mi menor', 'del nene', 'del nena'] },
    { subject: 'spouse', phrases: ['para mi esposa', 'para mi esposo', 'mi esposa', 'mi esposo', 'mi pareja'] },
    { subject: 'self', phrases: ['soy yo', 'es para mi '] },
  ];
  let lastMatchIndex = -1;
  let resolvedSubject = null;
  for (const entry of subjectPatterns) {
    for (const phrase of entry.phrases) {
      const phraseIndex = normalized.lastIndexOf(phrase);
      if (phraseIndex >= 0 && phraseIndex >= lastMatchIndex) {
        lastMatchIndex = phraseIndex;
...
function messageLooksLikePatientContextCorrection(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const normalized = normalizeForMatch(rawText);
  const correctionSignal =
    normalized.includes('en realidad') ||
    normalized.includes('no perdon') ||
    normalized.includes('no perdón') ||
    normalized.includes('me confundi') ||
    normalized.includes('me equivoque') ||
    normalized.includes('me equivoqué') ||
    (normalized.includes('no') && normalized.includes('soy yo'));
  if (!correctionSignal) return false;
  return resolvePatientSubjectFromMessage(rawText) !== null;
}

function buildPatientSubjectStatePatch(patientSubject) {
  if (!patientSubject) return {};
  return {
    patientSubject,
    patientSubjectUpdatedAtMs: Date.now(),
  };
}

function buildPatientSubjectCorrectionAcknowledgement(rawText) {
  const patientSubject = resolvePatientSubjectFromMessage(rawText);
  if (patientSubject === 'child') {
    return 'Perfecto, entonces es para tu hijo/a.';
  }
  if (patientSubject === 'spouse') {
    return 'Perfecto, entonces es para tu esposo/a.';
  }
  return 'Perfecto, entonces es para vos.';
}

function messageMentionsMixedCoupleCoverage(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const normalized = normalizeForMatch(rawText);
  const mentionsSpouse =
    normalized.includes('esposa') ||
    normalized.includes('esposo') ||
    normalized.includes('pareja');
  const mentionsParticular = normalized.includes('particular');
  ...
  return mentionsSpouse && mentionsParticular && mentionsCoverage;
}

function messageLooksLikeCoupleConsultationInquiry(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const normalized = normalizeForMatch(rawText);
  const coupleContext =
    normalized.includes('mi esposa') ||
    normalized.includes('mi esposo') ||
    normalized.includes('mi pareja') ||
    (normalized.includes('los dos') &&
      (normalized.includes('consult') || normalized.includes('turno') || normalized.includes('atend')));
  if (!coupleContext) return false;
  return (
    messageLooksLikeBookingIntent(rawText) ||
    normalized.includes('mismo dia') ||
    normalized.includes('mismo día') ||
    messageMentionsMixedCoupleCoverage(rawText) ||
    messageLooksLikeAnyPriceQuestion(rawText)
  );
}
...
function findSedeFromText(rawText) {
  if (!rawText || typeof rawText !== 'string') return null;
  if (messageMentionsAmbiguousChacoRegion(rawText)) return null;
...
function findAllSedeEntriesFromTextInOrder(rawText) {
  if (!rawText || typeof rawText !== 'string') return [];
  if (messageMentionsAmbiguousChacoRegion(rawText)) return [];
  const normalized = normalizeForMatch(rawText);
  const matches = [];
  for (const entry of SEDE_ENTRIES) {
    let earliestIndex = -1;
    for (const keyword of entry.match) {
      if (/^[12]$/.test(keyword)) continue;
      const keyNorm = normalizeForMatch(keyword);
      const index = normalized.indexOf(keyNorm);
      if (index >= 0 && (earliestIndex < 0 || index < earliestIndex)) {
        earliestIndex = index;
      }
    }
    if (earliestIndex >= 0) {
      matches.push({ entry, index: earliestIndex });
    }
  }
  ...
  for (const match of matches) {
    if (seenDisplayNames.has(match.entry.displayName)) continue;
    seenDisplayNames.add(match.entry.displayName);
    orderedEntries.push(match.entry);
  }
...
async function sendMedicalEmergencyResponse(from, priorState, profileDisplayName) {
  ...
  await setConversationState(
    ...
    mergeConversationStatePreservingGreeting(priorState, priorState || {}, {
      ...
      lastMedicalEmergencyGuidanceAtMs: Date.now(),
...
function priorStateHasRecentMedicalEmergencyGuidance(priorState) {
  if (!priorState || typeof priorState !== 'object') return false;
  const guidedAtMs = Number(priorState.lastMedicalEmergencyGuidanceAtMs);
  if (
    Number.isFinite(guidedAtMs) &&
    Date.now() - guidedAtMs <= MEDICAL_EMERGENCY_GUIDANCE_CONTEXT_WINDOW_MS
  ) {
    return true;
  }
  const lastBotReplyText =
  ...
  return (
    ...
    Number.isFinite(lastBotReplyAtMs) &&
    Date.now() - lastBotReplyAtMs <= MEDICAL_EMERGENCY_GUIDANCE_CONTEXT_WINDOW_MS &&
    (lastBotReplyText.includes('107') || lastBotReplyText.includes('guardia'))
  );
}

function messageInsistsOnSameDayDoctorVisit(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const normalized = normalizeForMatch(rawText);
  const mentionsToday =
  ...
  const wantsDoctorVisit =
    normalized.includes('doctor') ||
    normalized.includes('dr liber') ||
    normalized.includes('dr. liber') ||
    normalized.includes('verlo') ||
    normalized.includes('verme') ||
    normalized.includes('quiero ver') ||
    normalized.includes('necesito ver') ||
    normalized.includes('puedo ver') ||
    normalized.includes('turno') ||
...
async function tryHandleSameDayDoctorVisitInquiry(from, bodyText, priorState, profileDisplayName) {
  ...
  const mergedState = mergeConversationStatePreservingGreeting(
    ...
    priorState || {},
    {
      ...
      lastSameDayDoctorVisitHandledAtMs: Date.now(),
    }
  );
  ...
  if (lastSede) {
    ...
    const stateAfterEmpathy = mergeConversationStatePreservingGreeting(
      priorState,
      priorState || {},
      mergedState
    );
...
function messageMentionsExistingBookingContext(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const normalized = normalizeForMatch(rawText)
    .replace(/[!?.,;:]+/g, ' ')
    .replace(/\s+/g, ' ')
  ...
  return (
    ...
    normalized.includes('saque un turno') ||
    normalized.includes('saqué un turno') ||
    normalized.includes('saque turno') ||
    normalized.includes('saqué turno') ||
...
function messageAsksAboutExistingBookingScheduleRecall(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  if (!messageMentionsExistingBookingContext(rawText)) return false;
  const normalized = normalizeForMatch(rawText)
    .replace(/[!?.,;:]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  ...
  const asksRecall =
    ...
    normalized.includes('olvide cuando') ||
    normalized.includes('olvidé cuándo') ||
    normalized.includes('olvide que dia') ||
    normalized.includes('olvidé qué día') ||
    normalized.includes('que dia era') ||
    normalized.includes('qué día era') ||
    normalized.includes('que dia es') ||
    normalized.includes('qué día es') ||
    normalized.includes('si era el') ||
    normalized.includes('si es el') ||
    (Boolean(extractWeekdayNameFromText(rawText)) && normalized.includes(' o '));
  return asksRecall || (normalized.includes('turno') && normalized.includes('no recuerdo'));
}

function messageAsksToConfirmExistingBooking(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  if (messageAsksAboutExistingBookingScheduleRecall(rawText)) return true;
  const normalized = normalizeForMatch(rawText)
    .replace(/[!?.,;:]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  ...
  const hasBookedContext = messageMentionsExistingBookingContext(rawText);
...
function messageAsksWhyChooseDoctorOrTrustQuestion(rawText) {
  ...
  const asksDifferentApproach =
    ...
    normalized.includes('en que se diferencia') ||
    normalized.includes('qué se diferencia');
  ...
  if (
    asksDifferentApproach &&
    (normalized.includes('dr') || normalized.includes('doctor') || normalized.includes('liber'))
  ) {
    return true;
  }
  if (mentionsLongFailedTreatmentWithAlergyContext(normalized)) {
    return true;
  }
  return false;
}

function mentionsLongFailedTreatmentWithAlergyContext(normalized) {
  if (!normalized || typeof normalized !== 'string') return false;
  const mentionsAllergyContext = normalized.includes('alerg');
  const mentionsFailedHistory =
    normalized.includes('nada funcion') ||
    normalized.includes('nada me funcion') ||
    normalized.includes('anos que me atiendo') ||
    normalized.includes('años que me atiendo') ||
...
function messageLooksLikeOsVersusPrivateGuidanceQuestion(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const normalized = normalizeForMatch(rawText);
  return (
    normalized.includes('que me conviene') ||
    normalized.includes('qué me conviene') ||
    normalized.includes('no se si usar') ||
    normalized.includes('no sé si usar') ||
    normalized.includes('obra social o particular') ||
    normalized.includes('particular u obra') ||
    normalized.includes('particular o obra') ||
    normalized.includes('usar mi obra social o pagar') ||
    normalized.includes('usar la obra social o pagar')
  );
}

function messageLooksLikeStudyProcedureRequest(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const normalized = normalizeForMatch(rawText);
  const wantsProcedure =
    normalized.includes('quiero hacerme') ||
    normalized.includes('necesito hacerme') ||
    normalized.includes('quisiera hacerme') ||
    normalized.includes('me quiero hacer') ||
    normalized.includes('me tengo que hacer');
  if (!wantsProcedure) return false;
  return (
    messageMatchesStudiesTopic(rawText) ||
    Boolean(getStudyTypeFromText(rawText)) ||
    messageLooksLikeVagueOrMisspelledStudyMention(rawText)
  );
}

function messageLooksLikeReferralSedePatientGuidanceInquiry(rawText, priorState = null) {
  if (!rawText || typeof rawText !== 'string') return false;
  const sedeEntry = findSedeFromText(rawText);
  if (!sedeEntry || !isReferralOnlySedeEntry(sedeEntry)) return false;
  const normalized = normalizeForMatch(rawText);
  const hasStudySignal =
    Boolean(getStudyTypeFromText(rawText)) ||
    messageAsksAboutStudiesOrTests(rawText) ||
    messageLooksLikeVagueOrMisspelledStudyMention(rawText);
  const hasHealthInsuranceSignal =
    messageLooksLikeHealthInsurancePlusQuestion(rawText) ||
    messageStatesHealthInsuranceMembership(rawText) ||
    Boolean(tryExtractHealthInsuranceName(rawText));
  const hasGuidanceSignal =
    messageLooksLikeOsVersusPrivateGuidanceQuestion(rawText) ||
    messageAsksAboutConditionTreatment(rawText) ||
    normalized.includes('lloran') ||
    normalized.includes('ojos') ||
    normalized.includes('picazon') ||
    normalized.includes('picazón') ||
    normalized.includes('estornud');
  const topicSignals = [hasStudySignal, hasHealthInsuranceSignal, hasGuidanceSignal].filter(Boolean).length;
  if (topicSignals >= 2) return true;
  if (hasStudySignal && hasHealthInsuranceSignal) return true;
  if (hasGuidanceSignal && hasHealthInsuranceSignal && hasStudySignal) return true;
  return false;
}

function buildOsVersusPrivateGuidanceReply(sedeEntry, healthInsuranceName) {
  const cityName = sedeEntry && sedeEntry.displayName ? sedeEntry.displayName : 'tu ciudad';
  if (healthInsuranceName) {
    return `Con ${healthInsuranceName} en ${cityName} podés atender por prepaga según tu cobertura; la opción particular sirve si preferís no usar la prepaga o necesitás algo fuera de plan. En la consulta de evaluación te orientan qué te conviene según tu caso.`;
  }
  return `Podés atender por obra social/prepaga o particular; en la consulta de evaluación te orientan según tu cobertura y lo que necesites.`;
}

async function buildReferralSedePatientGuidanceReplies(
  priorState,
  rawText,
  sedeEntry,
  healthInsuranceName,
  ...
  const replies = [];
  const studyType = await resolveStudyTypeFromMessage(rawText, priorState, { profileDisplayName });
  const normalized = normalizeForMatch(rawText);

  if (studyType) {
    replies.push(buildStudyAvailabilityPrimaryReply(studyType, priorState, { sedeEntry }));
    if (isAllergyStudyType(studyType) || isPatchStudyType(studyType)) {
      replies.push(
        'Para el test de alergia primero hace falta una consulta de evaluación: el Dr. ve tu caso y ahí define qué estudio corresponde y el costo según tu cobertura.'
      );
    }
  }

  if (healthInsuranceName) {
    replies.push(await buildHealthInsuranceCoverageLineForSede(sedeEntry, healthInsuranceName));
  }

  if (messageLooksLikeOsVersusPrivateGuidanceQuestion(rawText)) {
    replies.push(buildOsVersusPrivateGuidanceReply(sedeEntry, healthInsuranceName));
  } else if (normalized.includes('lloran') || normalized.includes('ojos')) {
    replies.push(
      'Con ojos que lloran hace tiempo suele convenir empezar por la consulta de evaluación; ahí el Dr. orienta si el test de alergia es lo indicado y cómo usar tu cobertura.'
    );
  }
...
async function tryHandleReferralSedePatientGuidanceInquiry(from, bodyText, priorState, profileDisplayName) {
  ...
  const sedeEntry = patientContext.sedeEntry || findSedeFromText(bodyText);
  if (!sedeEntry || !isReferralOnlySedeEntry(sedeEntry)) return false;
  const healthInsuranceName =
    ...
    resolveActiveHealthInsuranceNameFromState(mergedState);
  const replies = await buildReferralSedePatientGuidanceReplies(
    mergedState,
    bodyText,
    sedeEntry,
    healthInsuranceName,
  ...
  return deliverSequentialPatientTextMessages(from, replies, priorState, profileDisplayName, {
    ...(patientContext.statePatch || {}),
    ...
    userMessage: bodyText,
    replyContext: 'referral_sede_guidance',
    suppressBookingLinkOffer: true,
  });
}
...
function messageLooksLikeReferralOnlySedeBookingIntent(rawText, priorState = null) {
  if (!rawText || typeof rawText !== 'string') return false;
  if (messageLooksLikeReferralSedePatientGuidanceInquiry(rawText, priorState)) return false;
...
async function sendReferralOnlySedeBookingReply(from, sedeEntry, priorState, profileDisplayName, bodyText = '') {
  ...
  if (bodyText) {
    ...
    patientContextPatch = {
      ...(patientContext.statePatch || {}),
      ...(patientContext.healthInsuranceName ? buildLastHealthInsuranceDiscussionStatePatch() : {}),
    };
  }
  const wrapped = buildAutoReplyWithGreetingIfNeeded(reply, profileDisplayName, priorState);
  await setConversationState(
...
function guessKeyLooksLikePrivatePayIntent(guessKey) {
  ...
  if (
    normalized.includes('hacerlo particular') ||
    normalized.includes('quiero hacerlo particular') ||
    normalized.includes('si quiero particular') ||
    normalized.includes('lo quiero particular')
  ) {
    return true;
  }
  return false;
...
async function isHealthInsuranceAcceptedInSede(sedeEntry, healthInsuranceName) {
  if (!sedeEntry || !healthInsuranceName) return false;
  const canonicalHealthInsuranceName =
    normalizeHealthInsuranceCanonicalName(healthInsuranceName) || healthInsuranceName;
  if (isKnownNotAcceptedHealthInsurance(canonicalHealthInsuranceName)) return false;

  const plusRule = await lookupPlusRule(sedeEntry.displayName, canonicalHealthInsuranceName);
  if (plusRule) return Boolean(plusRule.isAccepted);

  const cityNormalized = normalizeForMatch(sedeEntry.displayName);
  const healthInsuranceNormalized = normalizeForMatch(healthInsuranceName);
  const isOsde = healthInsuranceNormalized.includes('osde');
  const isIsunne = healthInsuranceNormalized.includes('isunne');
  const isSancor =
    healthInsuranceNormalized.includes('sancor') && !healthInsuranceNormalized.includes('mutual');
  return (
...
async function shouldIncludePrivateConsultationPriceReply(sedeEntry, healthInsuranceName, rawText) {
  if (!sedeEntry) return true;
  if (messageExplicitlyAsksPrivateConsultationPrice(rawText)) return true;
  if (!healthInsuranceName) return true;
  const asksConsultationPrice =
    messageAsksConsultationCostInText(rawText) ||
    messageAsksGenericConsultationPrice(rawText) ||
    messageAsksApproximateConsultationCost(rawText);
  if (!asksConsultationPrice) return true;
  const accepted = await isHealthInsuranceAcceptedInSede(sedeEntry, healthInsuranceName);
  return !accepted;
}
...
async function tryHumanizePatientReplyWithOpenAi(originalReply, options = {}) {
  ...
  if (options.replyContext === 'patient_topics_orchestrator') {
    replyContextInstructions.push(
      'Contexto: el paciente hizo varias preguntas en un mensaje (qué llevar, orden, autorización, pago, pediatría, duración, estudios el mismo día, etc.).',
      'Mantené EXACTOS todos los datos del borrador. No inventes políticas clínicas ni confirmes estudios el mismo día como regla fija.',
      'Respondé cada tema con naturalidad; no omitas bloques del borrador.'
    );
  }
  if (options.replyContext === 'patient_context_correction') {
    replyContextInstructions.push(
      'Contexto: el paciente corrigió para quién es la consulta (hijo/a, esposo/a o él/ella).',
      'Mantené el reconocimiento de la corrección y todos los datos factuales del borrador (sede, cobertura, condición).'
    );
  }
  if (options.replyContext === 'couple_consultation_guidance') {
    replyContextInstructions.push(
      'Contexto: pareja que quiere consultar los dos (a veces con coberturas distintas o el mismo día).',
      'Mantené: un turno por persona, no reservar por chat, no confirmar horarios. Respetá plus/cobertura y precios del borrador.'
    );
  }
  if (options.replyContext === 'spirometry_scheduling_inquiry') {
    replyContextInstructions.push(
      'Contexto: el paciente quiere espirometría en una sede con día u horario preferido.',
      'Mantené que el Dr. realiza la espirometría. No confirmes horarios puntuales ni disponibilidad del lunes/hora indicados.',
      'Mantené el link de agenda si el borrador lo incluye.'
    );
  }
  if (options.replyContext === 'reschedule_booking') {
    replyContextInstructions.push(
  ...
  if (options.replyContext === 'change_booking_sede') {
    replyContextInstructions.push(
      'Contexto: el paciente ya tiene turno en una sede y quiere atenderse en otra (cambio de sede).',
      'Mantené EXACTOS los teléfonos de sede del borrador. No ofrezcas link de agenda ni generes un turno nuevo por chat.'
    );
  }
  if (options.replyContext === 'confirm_existing_booking') {
    replyContextInstructions.push(
      'Contexto: el paciente ya reservó turno y pide confirmar, verificar la reserva, o no recuerda el día (ej. jueves o viernes).',
      'Mantené EXACTOS los teléfonos de sede del borrador. No consultes la agenda ni confirmes horarios por chat; derivá al teléfono de la sede.'
    );
  }
  if (options.replyContext === 'chaco_sede_disambiguation') {
    replyContextInstructions.push(
      'Contexto: el paciente dijo Chaco sin ciudad clara; hay que saber si es Resistencia o Sáenz Peña.',
  ...
  if (options.replyContext === 'complete_cost_total') {
    replyContextInstructions.push(
      'Contexto: el paciente pidió el TOTAL (consulta + estudio, particular o con obra social).',
      'Mantené EXACTOS todos los montos y el desglose del borrador. Si hay total aproximado, decilo con claridad.',
      'Tono cálido y claro; no mandes al paciente a "escribir de nuevo" si el borrador ya trae el total.',
      'No ofrezcas link de agenda en esta respuesta salvo que el borrador ya lo incluya.'
    );
  }
...
function messagePrioritizesBookingOverHealthInsuranceInquiry(rawText) {
  ...
  const hasBookingSignal =
    messageLooksLikeBookingIntent(rawText) ||
    messageLooksLikeSpecificSlotBookingRequest(rawText) ||
    messageExplicitlyRequestsBookingLink(rawText);
  if (!hasBookingSignal) return false;
  ...
  const mentionsHealthInsurance =
    messageLooksLikeHealthInsurancePlusQuestion(rawText) ||
    messageStatesHealthInsuranceMembership(rawText) ||
    Boolean(tryExtractHealthInsuranceName(rawText));
  return mentionsHealthInsurance;
}
...
function collapseRedundantPatientQuestionIntents(intents, options = {}) {
  ...
  if (options.isStandaloneSpirometryWithoutConsultation) {
    collapsed = collapsed.filter(
      (intent) =>
        intent !== 'CONSULTATION_AND_STUDY_PRICE' &&
        intent !== 'CONSULTATION_PRICE' &&
        intent !== 'STUDY_PRICE'
    );
    if (!collapsed.includes('STANDALONE_SPIROMETRY_PRICE')) {
      collapsed.push('STANDALONE_SPIROMETRY_PRICE');
    }
  }
  if (
    collapsed.includes('CONSULTATION_PRICE') &&
    (collapsed.includes('HEALTH_INSURANCE_ACCEPTANCE') || collapsed.includes('HEALTH_INSURANCE_PLUS'))
  ) {
    collapsed = collapsed.filter((intent) => intent !== 'CONSULTATION_PRICE');
  }
...
function messageLooksLikeMultiQuestionPatientInquiry(rawText, priorState = null) {
  ...
  const patientTopics = collectPatientReplyTopicsFromMessage(rawText, priorState);
  if (patientTopics.length >= 2) return true;

  if (questionCount >= 1 && messageHasMultipleDistinctQuestionSignals(rawText)) return true;

  if (inferPatientQuestionIntentsFromMessage(rawText).length >= 2) return true;
...
function classifyPatientQuestionSegmentIntent(segmentText, fullText) {
  ...
  if (
    (segment.includes('atienden') ||
      ...
      segment.includes('cubren')) &&
    hasHealthInsuranceContext
  ) {
    return 'HEALTH_INSURANCE_ACCEPTANCE';
  }
  if (
    messageAsksAboutStandaloneSpirometryWithoutConsultation(fullText) &&
    (segment.includes('sin consulta') ||
      segment.includes('puedo hacer') ||
      segment.includes('puede hacer') ||
      segment.includes('hacerla sin'))
  ) {
  ...
  if (
    messageAsksAboutStandaloneSpirometryWithoutConsultation(fullText) &&
    messageLooksLikeAnyPriceQuestion(segmentText)
  ) {
    return 'STANDALONE_SPIROMETRY_PRICE';
  }
  if (messageLooksLikeAnyPriceQuestion(segmentText) && messageMatchesStudiesTopic(fullText)) {
    return 'CONSULTATION_AND_STUDY_PRICE';
  }
  if (
    messageLooksLikeRealtimeAvailabilityQuestion(segmentText) ||
    messageLooksLikeRealtimeAvailabilityQuestion(fullText) ||
    messageLooksLikeScheduleAvailabilityQuestion(segmentText) ||
    messageLooksLikeBookingIntent(segmentText)
  ) {
  ...
  if (messageLooksLikeAnyPriceQuestion(segmentText) && segment.includes('consulta')) {
    if (
      hasHealthInsuranceContext &&
      !messageExplicitlyAsksPrivateConsultationPrice(fullText)
    ) {
      return 'HEALTH_INSURANCE_PLUS';
    }
  ...
  if (messageAsksAboutConsultDuration(segmentText) || messageAsksAboutConsultDuration(fullText)) {
    return 'CONSULTATION_DURATION';
  }
  if (
    messageAsksWhetherConsultationIncludesStudies(segmentText) ||
    messageAsksWhetherConsultationIncludesStudies(fullText)
  ) {
    return 'CONSULTATION_INCLUDES_STUDIES';
  }
...
const PATIENT_QUESTION_INTENT_ORDER = [
  ...
  'CONSULTATION_INCLUDES_STUDIES',
  'SEDE_PRESENCE',
  'HEALTH_INSURANCE_ACCEPTANCE',
  'HEALTH_INSURANCE_PLUS',
  'STUDY_AVAILABILITY',
  'STANDALONE_SPIROMETRY_PRICE',
...
async function extractPatientQuestionIntentsWithOpenAi(userMessage, priorState, profileDisplayName) {
  ...
  const systemPrompt = [
    ...
    'Reglas:',
    '- Si pregunta si ACEPTAN/ATIENDEN o si CUBRE una obra social o prepaga → HEALTH_INSURANCE_ACCEPTANCE (no SEDE_PRESENCE).',
    '- Si pregunta cuánto dura la consulta o el turno → CONSULTATION_DURATION.',
    '- Si pregunta si la consulta incluye estudios/tests → CONSULTATION_INCLUDES_STUDIES.',
    '- Si pregunta ayuno o preparación del estudio → STUDY_PREPARATION.',
    ...
    '- Si pregunta cuánto sale/cuesta la consulta y nombra obra social/prepaga (sin pedir particular) → HEALTH_INSURANCE_PLUS o HEALTH_INSURANCE_ACCEPTANCE, NO CONSULTATION_PRICE.',
    '- Si pide SOLO espirometría sin consulta y pregunta precio → STANDALONE_SPIROMETRY_PRICE.',
    '- Si pregunta turnos/disponibilidad/esta semana → BOOKING_AVAILABILITY.',
...
async function buildReplyForPatientQuestionIntent(intent, context) {
  ...
  switch (intent) {
    ...
    case 'CONSULTATION_DURATION':
      return CONSULT_DURATION_MESSAGE;
    case 'CONSULTATION_INCLUDES_STUDIES':
      return CONSULTATION_DOES_NOT_INCLUDE_STUDIES_MESSAGE;
    case 'SEDE_PRESENCE':
    ...
    case 'CONSULTATION_PRICE': {
      if (
        healthInsuranceName &&
        !(await shouldIncludePrivateConsultationPriceReply(lastSede, healthInsuranceName, fullText))
      ) {
        return await buildConsultationPriceReplyForSedeAndHealthInsurance(
          lastSede,
          healthInsuranceName,
          priorState
        );
    ...
    case 'STANDALONE_SPIROMETRY_PRICE':
      return buildSpirometryStandalonePriceReply(priorState, fullText, lastSede);
    case 'BOOKING_AVAILABILITY':
      return 'Por acá no confirmamos cupo de esta semana; en la agenda online ves los turnos disponibles.';
    case 'STUDY_PRICE': {
      const studyType = await resolveStudyTypeFromMessage(fullText, priorState, { profileDisplayName });
...
async function tryHandleMultiQuestionPatientInquiryWithOpenAi(
  ...
  const isStandaloneSpirometryWithoutConsultation =
    messageAsksAboutStandaloneSpirometryWithoutConsultation(bodyText);
  intents = collapseRedundantPatientQuestionIntents(intents, {
    isGenericInstitutionHealthInsurance,
    isStandaloneSpirometryWithoutConsultation,
  });

  const patientTopics = collectPatientReplyTopicsFromMessage(bodyText, mergedState);
  const topicsCoveredByIntents = new Set(
    intents
      .map((intent) => {
        if (intent === 'PEDIATRICS') return 'PEDIATRICS';
        if (intent === 'CONSULTATION_DURATION') return 'CONSULTATION_DURATION';
        if (intent === 'STUDY_PREPARATION') return 'STUDY_PREPARATION';
        return null;
      })
      .filter(Boolean)
  );
  const supplementalPatientTopics = patientTopics.filter((topic) => !topicsCoveredByIntents.has(topic));

  if (intents.length === 0 && supplementalPatientTopics.length < 2 && patientTopics.length < 2) {
    return false;
  }

  const shouldDeliverBookingLink =
    intents.includes('BOOKING_AVAILABILITY') &&
    lastSede &&
    !isReferralOnlySedeEntry(lastSede);
  const intentsForTextReplies = intents.filter((intent) => intent !== 'BOOKING_AVAILABILITY');
  ...
  for (const intent of intentsForTextReplies) {
  ...
  if (supplementalPatientTopics.length > 0) {
    const topicReplies = await buildRulesReplyPlanFromTopics(
      mergedState,
      bodyText,
      lastSede,
      healthInsuranceName,
      supplementalPatientTopics
    );
    for (const topicReply of topicReplies) {
      const trimmedTopicReply = stripDuplicatePatientReplyPrefix(topicReply.trim(), seenReplyTexts);
      if (!trimmedTopicReply || seenReplyTexts.has(trimmedTopicReply)) continue;
      seenReplyTexts.add(trimmedTopicReply);
      replies.push(trimmedTopicReply);
    }
  }
  ...
  if (
    isStandaloneSpirometryWithoutConsultation &&
    !shouldDeliverBookingLink &&
    lastSede &&
    !isReferralOnlySedeEntry(lastSede)
  ) {
    const bookingOfferReply = buildMicroCommitmentMessage();
    if (!seenReplyTexts.has(bookingOfferReply)) {
  ...
  if (shouldDeliverBookingLink) {
    const stateAfterReplies = mergeConversationStatePreservingGreeting(
      priorState,
      priorState || {},
      statePatch
    );
    await deliverBookingLinkReply(from, lastSede, stateAfterReplies, profileDisplayName, {
      userMessage: bodyText,
      forceResend: true,
      sendFollowUp: false,
    });
...
function messageConfirmsLinkSend(rawText) {
  ...
  if (/^(si|dale|ok|oka|de una|listo|ya)\b/.test(normalized)) {
    if (
      normalized.includes('particular') ||
      normalized.includes('total') ||
...
async function tryHandleAwaitingLinkConfirmation(from, bodyText, priorState, profileDisplayName) {
  ...
  const shouldBypassPendingLinkConfirmation =
    ...
    (messageLooksLikeMultiQuestionPatientInquiry(bodyText, priorState) ||
...
async function sendConsultationPriceQuestionReply(from, bodyText, priorState, profileDisplayName) {
  ...
  const healthInsuranceName =
    patientContext.healthInsuranceName || resolveActiveHealthInsuranceNameFromState(mergedState);
...
async function tryResolveSedeFromTextWithOpenAi(rawText) {
  ...
  const systemPrompt = [
    ...
    'Si solo dice "Chaco" o "provincia del Chaco" sin Resistencia ni Sáenz Peña, devolvé UNKNOWN.',
...
async function tryExtractPatientContextWithOpenAi(rawText, priorState) {
  ...
  const systemPrompt = [
    ...
    'Si solo dice Chaco sin ciudad, city=UNKNOWN.',
    'patientSubject opcional: self, child, spouse si corrige para quién es la consulta.',
  ].join('\n');
...
async function resolvePatientContextFromMessage(rawText, priorState, options = {}) {
  ...
  const statePatch = {
    ...
    ...(healthInsuranceName
      ...
      : {}),
    ...(patientSubject ? buildPatientSubjectStatePatch(patientSubject) : {}),
  };
  return { sedeEntry, healthInsuranceName, statePatch, patientSubject };
}
...
async function tryResolveHealthInsurancePlusIntentWithOpenAi(userMessage, options = {}) {
  if (messagePrioritizesBookingOverHealthInsuranceInquiry(userMessage)) return false;
  const apiKey = getOpenAiApiKey();
  if (apiKey) {
    const modelName = getOpenAiModelName();
    const systemPrompt = [
      ...
      'NO si menciona obra social/prepaga solo como contexto pero pide TURNO o agenda ("tengo IOSCOR y quiero turno el martes").',
...
async function tryHandleBookingWithHealthInsuranceContext(
  from,
  bodyText,
  priorState,
  profileDisplayName
...
async function tryHandleBookingWithPatientContext(from, bodyText, priorState, profileDisplayName) {
  ...
  const bookingStatePatch = buildFreshBookingWithoutSedeStatePatch(bodyText);
  const bookingState = mergeConversationStatePreservingGreeting(
    mergedState,
    mergedState || {},
    bookingStatePatch
  );
  await setConversationState(from, bookingState);
  if (
...
function messageLooksLikeMultiIntentCandidate(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  if (messageLooksLikePatientTopicsOrchestratorCandidate(rawText)) return true;
  if (messageLooksLikeComplexClinicalInquiry(rawText)) return true;
...
async function tryHandleMultiIntentPatientInquiry(from, bodyText, priorState, profileDisplayName) {
  ...
  if (await tryHandlePatientTotalCostInquiries(from, bodyText, priorState, profileDisplayName)) {
    return true;
  }
  ...
  if (hasHealthInsurance && hasPrivatePrice) {
    ...
    if (sedeFromMessage && healthInsuranceName) {
      ...
      if (healthInsuranceSummary !== 'ASK_CITY_FOR_HEALTH_INSURANCE') {
        const includePrivatePrice = await shouldIncludePrivateConsultationPriceReply(
          sedeFromMessage,
          healthInsuranceName,
          bodyText
        );
        const privatePriceReply = includePrivatePrice ? await buildPrivatePriceReply(sedeFromMessage) : '';
        const combined = includePrivatePrice
          ? `${healthInsuranceSummary} ${privatePriceReply}`.trim()
...
async function dispatchOpenAiPrimaryIntent(from, bodyText, priorState, profileDisplayName, primaryIntent) {
  ...
  if (await tryHandleChacoSedeDisambiguationAnswer(from, bodyText, priorState, profileDisplayName)) {
    return true;
  }
  ...
  if (await tryHandleCoupleConsultationInquiry(from, bodyText, priorState, profileDisplayName)) {
    return true;
  }
  if (await tryHandleChangeBookingSedeInquiry(from, bodyText, priorState, profileDisplayName)) {
    return true;
  }
...
function resolvePatientConfirmedSedeEntryFromState(state) {
  if (!state || typeof state !== 'object') return null;
  if (typeof state.lastSedeEnvKey === 'string' && state.lastSedeEnvKey.trim().length > 0) {
    const entry = resolveSedeEntryByEnvKey(state.lastSedeEnvKey.trim());
    if (entry) return entry;
  }
  if (typeof state.lastSedeDisplayName === 'string' && state.lastSedeDisplayName.trim().length > 0) {
    const entry = SEDE_ENTRIES.find((item) => item.displayName === state.lastSedeDisplayName.trim());
    if (entry) return entry;
  }
  return null;
}

function resolveActiveBookingSedeEntryFromState(state) {
  const patientConfirmedSede = resolvePatientConfirmedSedeEntryFromState(state);
  if (patientConfirmedSede) return patientConfirmedSede;
  return resolveLastSedeEntryFromState(state);
}
...
async function tryHandleSmartOpenAiFallback(from, bodyText, priorState, profileDisplayName) {
  ...
  if (messageShouldSkipOpenAiAssistantFallback(bodyText, priorState)) {
    return false;
  }
...
function messageLooksLikeRealtimeAvailabilityQuestion(rawText) {
  ...
  const asksAvailability =
    ...
    normalized.includes('turnos esta semana') ||
    normalized.includes('turno esta semana') ||
    (normalized.includes('esta semana') && normalized.includes('turno'));
...
function messageLooksLikeBookingIntent(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  if (messageAsksToChangeBookingSede(rawText)) return false;
  if (messageAsksToRescheduleOrCancelBooking(rawText)) return false;
  if (messageLooksLikeStudyProcedureRequest(rawText)) return false;
...
function isSpirometryStudyType(studyType) {
  if (!studyType || typeof studyType !== 'string') return false;
  return normalizeForMatch(studyType).includes('espirometr');
}

function isAllergyStudyType(studyType) {
  if (!studyType || typeof studyType !== 'string') return false;
  const normalized = normalizeForMatch(studyType);
  return normalized.includes('alerg') || normalized.includes('prick');
}

function isPatchStudyType(studyType) {
  if (!studyType || typeof studyType !== 'string') return false;
  const normalized = normalizeForMatch(studyType);
  return normalized.includes('parche') || normalized.includes('patch');
}

function normalizeResolvedStudyTypeFromOpenAi(rawValue) {
  if (!rawValue || typeof rawValue !== 'string') return null;
  const normalized = normalizeForMatch(rawValue);
  if (!normalized) return null;
  if (normalized.includes('espirometr') || normalized.includes('espirimetr') || normalized.includes('pulmon')) {
    return STUDY_TYPE_SPIROMETRY;
  }
  if (normalized.includes('parche') || normalized.includes('patch')) {
    return STUDY_TYPE_PATCH_TEST;
  }
  if (
    normalized.includes('alerg') ||
    normalized.includes('prick') ||
    normalized.includes('cutane') ||
...
function messageLooksLikeVagueOrMisspelledStudyMention(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  if (getStudyTypeFromText(rawText)) return false;
  const normalized = normalizeForMatch(rawText);
  if (!normalized) return false;
  const vagueStudySignals = [
    'estudio',
    'estudios',
    'prueba',
    'test',
  ...
  return vagueStudySignals.some((signal) => normalized.includes(signal));
}

async function tryResolveStudyTypeFromMessageWithOpenAi(userMessage, options = {}) {
  const apiKey = getOpenAiApiKey();
  if (!apiKey) return null;
  if (!messageLooksLikeVagueOrMisspelledStudyMention(userMessage)) return null;

  const modelName = getOpenAiModelName();
  const systemPrompt = [
    'Sos un clasificador de estudios médicos para WhatsApp de un consultorio de alergia e inmunología en español rioplatense.',
    'Tarea: detectar si el paciente menciona un estudio que realiza el Dr., aunque esté mal escrito.',
    'Respondé SOLO JSON válido con esta forma exacta: {"studyType":"espirometría"|"test de alergia"|"test del parche"|null}',
    'espirometría: espirometría, espirometria, espirimetria, eperitometria, test respiratorio, función pulmonar, etc.',
    'test de alergia: prick test, test cutáneo, test de alergia a alimentos/medicamentos/aeroalérgenos, typos de prick/alergia.',
    'test del parche: patch test, test de parches.',
    'null: no menciona un estudio del consultorio, solo análisis de sangre, imágenes, o tema ajeno.',
    'Si hay duda entre espirometría y test de alergia, elegí el que mejor encaje con las palabras del paciente.',
  ].join('\n');
  ...
  try {
    ...
    return normalizeResolvedStudyTypeFromOpenAi(parsed.studyType);
  } catch (error) {
    console.error('OpenAI study type resolver failed', error);
    return null;
  }
}

async function resolveStudyTypeFromMessage(rawText, priorState = null, options = {}) {
  const fromRules = getStudyTypeFromText(rawText);
  if (fromRules) return fromRules;
  ...
  const fromState =
    ...
    (messageLooksLikeStudyPriceFollowUp(rawText, priorState) ||
      stateHasRecentStudyPriceContext(priorState) ||
      messageLooksLikeVagueOrMisspelledStudyMention(rawText))
      ? normalizeResolvedStudyTypeFromOpenAi(priorState.lastStudyType.trim()) ||
        priorState.lastStudyType.trim()
      : null;
  if (fromState && !messageLooksLikeVagueOrMisspelledStudyMention(rawText)) {
    return fromState;
  }

  if (getOpenAiApiKey() && messageLooksLikeVagueOrMisspelledStudyMention(rawText)) {
    const fromOpenAi = await tryResolveStudyTypeFromMessageWithOpenAi(rawText, {
      priorState,
      profileDisplayName: options.profileDisplayName,
    });
    if (fromOpenAi) return fromOpenAi;
...
function messageLooksLikeSpirometryPriceOnlyInquiry(rawText, studyType) {
  ...
  if (
    ...
    normalized.includes('mas el') ||
    normalized.includes('más el')
  ) {
    return false;
  }
  if (messageLooksLikeBookingIntent(rawText)) return false;
...
function buildAllergyTestRequiresEvaluationPriceReply(lastSede = null) {
  const sedeFromState = lastSede || null;
  if (sedeFromState) {
    return `${ALLERGY_TEST_REQUIRES_EVALUATION_PRICE_MESSAGE} En ${sedeFromState.displayName} el Dr. lo evalúa en consulta.`;
  }
...
function buildSpirometryStandalonePriceReply(priorState, rawText, lastSede = null, options = {}) {
  ...
  if (
    ...
    (normalized.includes('asma') && normalized.includes('vuelvo'))
  ) {
    reply = `${reply} ${SPIROMETRY_FOLLOW_UP_PRICE_NOTE}`;
  }
  return reply;
}
...
async function sendSpirometrySchedulingInquiryReply(from, bodyText, priorState, profileDisplayName) {
  const patientContext = await resolvePatientContextFromMessage(bodyText, priorState, { profileDisplayName });
  const mergedState = mergeConversationStatePreservingGreeting(
    priorState,
    priorState || {},
  ...
  const primaryReply = buildStudyAvailabilityPrimaryReply('espirometría', mergedState, { sedeEntry: lastSede });
  const statePatch = {
    ...(patientContext.statePatch || {}),
    ...(buildLastSedeStatePatch(lastSede) || {}),
  ...
  const delivered = await deliverSequentialPatientTextMessages(
    ...
    {
      userMessage: bodyText,
      replyContext: 'spirometry_scheduling_inquiry',
      skipHumanization: true,
    }
  );
  if (!delivered) return false;

  if (isReferralOnlySedeEntry(lastSede)) return true;
  ...
  await deliverBookingLinkReply(from, lastSede, stateAfterReplies, profileDisplayName, {
    ...
    primaryPrefix:
      buildPreferredSlotBookingAcknowledgementPrefix(lastSede, bodyText) ||
      'Por acá no confirmamos horarios puntuales; en la agenda online ves los turnos disponibles.',
  });
  return true;
}
...
async function sendSpirometryOnlyInquiryReply(from, bodyText, priorState, profileDisplayName) {
  ...
  const delivered = await deliverSequentialPatientTextMessages(
    ...
    {
      userMessage: bodyText,
      replyContext: 'spirometry_only_inquiry',
      skipHumanization: true,
    }
  );
  if (!delivered) return false;

  if (asksAvailability && !isReferralOnlySedeEntry(lastSede)) {
    const stateAfterReplies = mergeConversationStatePreservingGreeting(
      priorState,
      priorState || {},
      statePatch
    ...
    await deliverBookingLinkReply(from, lastSede, stateAfterReplies, profileDisplayName, {
      ...
      sendFollowUp: false,
      primaryPrefix:
        'Por acá no confirmamos cupo de esta semana; en la agenda online ves los turnos disponibles.',
    });
  }
  return true;
}
...
async function tryHandleSpirometryOnlyInquiry(from, bodyText, priorState, profileDisplayName) {
  if (messageLooksLikeSpirometrySchedulingInquiry(bodyText)) {
    return sendSpirometrySchedulingInquiryReply(from, bodyText, priorState, profileDisplayName);
  }
  const resolvedStudyType = await resolveStudyTypeFromMessage(bodyText, priorState, { profileDisplayName });
  const matchesSpirometryOnlyInquiry =
    messageLooksLikeSpirometryOnlyInquiry(bodyText) ||
    (isSpirometryStudyType(resolvedStudyType) &&
      messageLooksLikeSpirometryPriceOnlyInquiry(bodyText, resolvedStudyType));
  if (!matchesSpirometryOnlyInquiry) return false;
  return sendSpirometryOnlyInquiryReply(from, bodyText, priorState, profileDisplayName);
}
...
function messageMentionsInfantPatientContext(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const normalized = normalizeForMatch(rawText);
  return (
    /\bbebe\b/.test(normalized) ||
    /\bbebes\b/.test(normalized) ||
    normalized.includes('meses de vida') ||
    /\b\d{1,2}\s*meses\b/.test(normalized)
  );
}

function buildPediatricCareConfirmationReply(rawText) {
  const childAgeMonths = extractChildAgeMonthsFromMessage(rawText);
  if (Number.isFinite(childAgeMonths) && childAgeMonths > 0 && childAgeMonths < 24) {
    return `Sí, el Dr. atiende bebés desde los primeros meses (incluido un bebé de ${childAgeMonths} meses), adolescentes y adultos.`;
  }
  ...
  if (messageMentionsInfantPatientContext(rawText)) {
    return 'Sí, el Dr. atiende bebés desde los primeros meses de vida, adolescentes y adultos.';
  }
  return 'Sí, el Dr. atiende niños, adolescentes y adultos.';
}

function messageMentionsStudyTypeInMessage(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const normalized = normalizeForMatch(rawText);
  return (
    messageMentionsSpirometryStudy(rawText) ||
...
function messageAsksStudyPerformanceQuestionInMessage(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const normalized = normalizeForMatch(rawText);
  return (
    /\b(hacen|hacem|hace|hacemos|realizan|realiza|pueden hacer|se hace)\b/.test(normalized) ||
...
function messageAsksWhetherDoctorPerformsStudyInMessage(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  if (
    messageMentionsStudyTypeInMessage(rawText) &&
    messageAsksStudyPerformanceQuestionInMessage(rawText)
  ) {
    return true;
  }
...
function stripDuplicatePatientReplyPrefix(replyText, seenReplyTexts) {
  if (!replyText || typeof replyText !== 'string') return '';
  let trimmedReply = replyText.trim();
  if (!trimmedReply || !seenReplyTexts || seenReplyTexts.size === 0) return trimmedReply;
  for (const priorReply of seenReplyTexts) {
    if (!priorReply || priorReply === trimmedReply) {
      if (priorReply === trimmedReply) return '';
      continue;
    }
    if (trimmedReply.startsWith(priorReply)) {
      trimmedReply = trimmedReply
        .slice(priorReply.length)
        .trim()
        .replace(/^[.,;:]+/, '')
        .trim();
      break;
...
async function deliverSequentialPatientTextMessages(
  ...
  const seenReplyTexts = new Set();
  const validMessages = messageTexts
    .map((text) => (typeof text === 'string' ? text.trim() : ''))
    .filter((text) => text.length > 0)
    .map((text) => stripDuplicatePatientReplyPrefix(text, seenReplyTexts))
    .filter((text) => {
      if (!text || seenReplyTexts.has(text)) return false;
      seenReplyTexts.add(text);
      return true;
    });
  ...
  for (let index = 0; index < validMessages.length; index += 1) {
    const finalizedText = await finalizePatientReplyText(validMessages[index], {
      priorState: nextState,
      profileDisplayName,
      userMessage: deliveryOptions.userMessage || '',
...
function messageAsksCompleteOrTotalCost(rawText) {
  ...
  if (
    ...
    normalized.includes('cuanto me saldria en total') ||
    normalized.includes('cuanto saldria en total') ||
...
function extractChildAgeMonthsFromMessage(rawText) {
  if (!rawText || typeof rawText !== 'string') return null;
  const normalized = normalizeForMatch(rawText);
  const childAgeMatch = normalized.match(
    /(?:bebe|bebes|hijo|hija|nene|nena|menor)[^0-9]{0,24}(\d{1,2})\s*meses/
  );
  if (!childAgeMatch || !childAgeMatch[1]) return null;
  const childAgeMonths = Number(childAgeMatch[1]);
  return Number.isFinite(childAgeMonths) ? childAgeMonths : null;
}

function messageAsksAboutMedicalOrderRequirement(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const normalized = normalizeForMatch(rawText);
  return (
    normalized.includes('necesito orden') ||
    normalized.includes('hace falta orden') ||
    normalized.includes('necesito la orden') ||
    (normalized.includes('orden') &&
      (normalized.includes('necesit') || normalized.includes('hace falta') || normalized.includes('llevar')))
  );
}

function messageAsksWhatToBringForConsultation(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const normalized = normalizeForMatch(rawText);
  return (
...
function messageAsksAboutSameDayStudies(rawText) {
  ...
  const asksSameDay =
    normalized.includes('mismo dia') ||
    normalized.includes('mismo día') ||
    normalized.includes('el mismo dia') ||
    normalized.includes('el mismo día') ||
...
const PATIENT_REPLY_TOPIC_OPENAI_TOKENS = [
  'WHAT_TO_BRING',
  'REFERRAL_ORDER',
  'AUTHORIZATION',
  'PAYMENT_METHODS',
...
function collectPatientReplyTopicsFromMessage(rawText, priorState = null) {
  if (!rawText || typeof rawText !== 'string') return [];
  const normalized = normalizeForMatch(rawText);
  const topics = [];
  if (
    ...
    messageMentionsInfantPatientContext(rawText)
  ) {
    topics.push('PEDIATRICS');
  }
  if (messageAsksWhatToBringForConsultation(rawText)) {
    topics.push('WHAT_TO_BRING');
  }
  if (messageAsksAboutReferralOrPrescription(rawText) || messageAsksAboutMedicalOrderRequirement(rawText)) {
    topics.push('REFERRAL_ORDER');
  }
  if (normalized.includes('autoriz') || normalized.includes('credencial')) {
    topics.push('AUTHORIZATION');
  }
  if (messageAsksAboutPaymentMethods(rawText)) {
  ...
  if (messageAsksAboutConsultDuration(rawText)) {
    topics.push('CONSULTATION_DURATION');
  }
  if (messageAsksAboutSameDayStudies(rawText)) {
    topics.push('SAME_DAY_STUDIES');
  }
  if (messageAsksAboutStudyPreparation(rawText, priorState)) {
    topics.push('STUDY_PREPARATION');
  }
  if (messageAsksAboutVirtualVisit(rawText)) {
    topics.push('VIRTUAL_VISIT');
  }
  return PATIENT_REPLY_TOPIC_ORDER.filter((topic) => topics.includes(topic));
}

function messageLooksLikePatientTopicsOrchestratorCandidate(rawText, priorState = null) {
  if (!rawText || typeof rawText !== 'string') return false;
  if (messageLooksLikeRichPatientIntakeInquiry(rawText)) return false;
  if (messageLooksLikeCompleteCostTotalInquiry(rawText, priorState)) return false;
  if (messageLooksLikeComplexClinicalInquiry(rawText)) return false;
...
function patientReplyTopicWasExplicitlyAskedInMessage(rawText, topic, priorState = null) {
  if (!rawText || typeof rawText !== 'string') return false;
  if (topic === 'PEDIATRICS') {
    return (
      messageMentionsChildPatientContext(rawText) ||
      messageAsksIfDoctorTreatsChildren(rawText) ||
      messageMentionsInfantPatientContext(rawText) ||
      /\batiende\s+nin/.test(normalizeForMatch(rawText))
    );
  }
  if (topic === 'WHAT_TO_BRING') return messageAsksWhatToBringForConsultation(rawText);
  if (topic === 'REFERRAL_ORDER') {
    return (
      messageAsksAboutReferralOrPrescription(rawText) || messageAsksAboutMedicalOrderRequirement(rawText)
    );
  }
  if (topic === 'AUTHORIZATION') {
    const normalized = normalizeForMatch(rawText);
    return normalized.includes('autoriz') || normalized.includes('credencial');
  }
  if (topic === 'PAYMENT_METHODS') return messageAsksAboutPaymentMethods(rawText);
  if (topic === 'INVOICE') return messageAsksAboutInvoice(rawText);
  if (topic === 'CONSULTATION_DURATION') return messageAsksAboutConsultDuration(rawText);
  if (topic === 'SAME_DAY_STUDIES') return messageAsksAboutSameDayStudies(rawText);
  if (topic === 'STUDY_PREPARATION') return messageAsksAboutStudyPreparation(rawText, priorState);
  if (topic === 'VIRTUAL_VISIT') return messageAsksAboutVirtualVisit(rawText);
  return true;
}

function filterPatientReplyTopicsToExplicitlyAskedQuestions(rawText, topics, priorState = null) {
  return (topics || []).filter((topic) =>
    patientReplyTopicWasExplicitlyAskedInMessage(rawText, topic, priorState)
  );
}

function messageLooksLikeClinicalOrPricingPatientInquiryBeyondFaqTopics(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  const normalized = normalizeForMatch(rawText);
  return (
    messageLooksLikeAnyPriceQuestion(rawText) ||
...
async function extractReplyTopicsWithOpenAi(userMessage, priorState, profileDisplayName) {
  const apiKey = getOpenAiApiKey();
  if (!apiKey) return null;
  const modelName = getOpenAiModelName();
  const systemPrompt = [
    'You extract patient question TOPICS from a WhatsApp message to a clinic bot (Spanish, Argentina).',
    'Return ONLY valid JSON: { "topics": ["TOPIC_1", "TOPIC_2"] }',
    `Allowed topics: ${PATIENT_REPLY_TOPIC_OPENAI_TOKENS.join(', ')}.`,
    'Map questions to topics:',
    '- what to bring / estudios previos / DNI → WHAT_TO_BRING',
    '- orden médica / receta / derivación → REFERRAL_ORDER',
    '- autorización obra social / credencial → AUTHORIZATION',
    '- transferencia / cómo pago / medios de pago → PAYMENT_METHODS',
    '- factura → INVOICE',
    '- cuánto dura la consulta → CONSULTATION_DURATION',
    '- bebé / niños / edad en meses → PEDIATRICS',
    '- test y espirometría el mismo día → SAME_DAY_STUDIES',
    '- preparación / ayunas estudio → STUDY_PREPARATION',
    '- consulta virtual → VIRTUAL_VISIT',
    'Include ALL topics the patient asked. Max 6 topics. If none match, return { "topics": [] }.',
  ].join('\n');
  const userContent = buildOpenAiClassifierUserContent(userMessage, {
    profileDisplayName,
    conversationContext: priorState ? buildIntentRoutingOpenAiContext(priorState) : '',
  ...
  try {
    ...
    const cleaned = topics
      .map((value) => (typeof value === 'string' ? value.trim().toUpperCase() : ''))
      .filter((value) => PATIENT_REPLY_TOPIC_OPENAI_TOKENS.includes(value) && value !== 'OTHER');
    return PATIENT_REPLY_TOPIC_ORDER.filter((topic) => cleaned.includes(topic));
  } catch (error) {
    console.error('OpenAI patient topics extractor failed', error);
    return null;
  }
}

function mergePatientReplyTopics(ruleTopics, openAiTopics) {
  const merged = new Set([...(ruleTopics || []), ...(openAiTopics || [])]);
  return PATIENT_REPLY_TOPIC_ORDER.filter((topic) => merged.has(topic));
}

async function buildRulesReplyPlanFromTopics(priorState, rawText, lastSede, healthInsuranceName, topics) {
  const replies = [];
  const enrichedState = mergeConversationStatePreservingGreeting(
    priorState,
    priorState || {},
  ...
  for (const topic of topics) {
    ...
    if (topic === 'WHAT_TO_BRING') {
      replies.push(buildStudiesToBringReply(enrichedState, rawText));
      continue;
    }
    if (topic === 'REFERRAL_ORDER') {
    ...
    if (topic === 'AUTHORIZATION') {
      replies.push(AUTHORIZATION_AND_DIGITAL_CARD_MESSAGE.replace(' ¿Te sirve?', '.'));
      continue;
    }
    if (topic === 'PAYMENT_METHODS') {
      replies.push(PAYMENT_METHODS_MESSAGE.replace(' ¿Te sirve?', '.'));
      continue;
    }
    if (topic === 'INVOICE') {
      replies.push(INVOICE_MESSAGE.replace(' ¿Te sirve?', '.'));
      continue;
    }
    if (topic === 'CONSULTATION_DURATION') {
      replies.push(CONSULT_DURATION_MESSAGE);
      continue;
    }
    if (topic === 'SAME_DAY_STUDIES') {
      replies.push(SAME_DAY_STUDIES_MESSAGE);
      continue;
    }
    if (topic === 'STUDY_PREPARATION') {
      replies.push(buildStudyPreparationReply(enrichedState, rawText));
      continue;
    }
...
async function tryHandlePatientTopicsOrchestrator(from, bodyText, priorState, profileDisplayName) {
  if (!messageLooksLikePatientTopicsOrchestratorCandidate(bodyText, priorState)) return false;
  if (messageLooksLikeMultiQuestionPatientInquiry(bodyText, priorState)) return false;
  if (
    messageLooksLikeRichPatientIntakeInquiry(bodyText) ||
    messageLooksLikeCompleteCostTotalInquiry(bodyText, priorState) ||
    messageLooksLikeComplexClinicalInquiry(bodyText)
  ) {
    return false;
  }
  let topics = collectPatientReplyTopicsFromMessage(bodyText, priorState);
  const questionCount = (String(bodyText).match(/\?/g) || []).length;
  if (
    topics.length < 2 &&
    questionCount >= 3 &&
    getOpenAiApiKey() &&
    !messageLooksLikeClinicalOrPricingPatientInquiryBeyondFaqTopics(bodyText)
  ) {
    const openAiTopics = await extractReplyTopicsWithOpenAi(bodyText, priorState, profileDisplayName);
    if (openAiTopics && openAiTopics.length > 0) {
      topics = mergePatientReplyTopics(topics, openAiTopics);
    }
  }
  topics = filterPatientReplyTopicsToExplicitlyAskedQuestions(bodyText, topics, priorState);
  if (
    topics.length === 1 &&
    topics[0] === 'PEDIATRICS' &&
    messageLooksLikeClinicalOrPricingPatientInquiryBeyondFaqTopics(bodyText)
  ...
  const replies = await buildRulesReplyPlanFromTopics(
    mergedState,
    bodyText,
    lastSede,
    healthInsuranceName,
...
async function tryHandleChacoSedeDisambiguationAnswer(from, bodyText, priorState, profileDisplayName) {
  ...
  if (!sedeEntry) {
    ...
    const wrapped = buildAutoReplyWithGreetingIfNeeded(reprompt, profileDisplayName, priorState);
    await setConversationState(
      from,
      mergeConversationStatePreservingGreeting(
        priorState,
...
async function tryHandlePatientContextCorrection(from, bodyText, priorState, profileDisplayName) {
  if (!messageLooksLikePatientContextCorrection(bodyText)) return false;
  const patientSubject = resolvePatientSubjectFromMessage(bodyText);
  if (!patientSubject) return false;
  const subjectStatePatch = buildPatientSubjectStatePatch(patientSubject);
  const mergedState = mergeConversationStatePreservingGreeting(
    priorState,
    priorState || {},
    subjectStatePatch
  );
  const hasFurtherClinicalQuestions =
    messageLooksLikeRichPatientIntakeInquiry(bodyText) ||
    messageLooksLikeComplexClinicalInquiry(bodyText) ||
    messageLooksLikePatientTopicsOrchestratorCandidate(bodyText, mergedState) ||
    messageAsksAboutConditionTreatment(bodyText) ||
    findSedeFromText(bodyText) ||
    tryExtractHealthInsuranceName(bodyText);
  if (hasFurtherClinicalQuestions) {
    if (await tryHandleRichPatientIntakeInquiry(from, bodyText, mergedState, profileDisplayName)) {
      return true;
    }
    if (await tryHandleComplexClinicalInquiry(from, bodyText, mergedState, profileDisplayName)) {
      return true;
    }
    if (await tryHandlePatientTopicsOrchestrator(from, bodyText, mergedState, profileDisplayName)) {
      return true;
    }
    const patientContext = await resolvePatientContextFromMessage(bodyText, mergedState, { profileDisplayName });
    const lastSede = patientContext.sedeEntry || resolveLastSedeEntryFromState(mergedState);
    if (lastSede && messageAsksAboutConditionTreatment(bodyText)) {
      const ack = buildPatientSubjectCorrectionAcknowledgement(bodyText);
      const conditionReply = buildConditionTreatmentReply(mergedState, bodyText);
      const replies = [`${ack} ${conditionReply}`];
      const healthInsuranceName = await resolveHealthInsuranceNameFromMessage(bodyText, mergedState, {
        profileDisplayName,
      });
      if (healthInsuranceName) {
  ...
  const acknowledgement = `${buildPatientSubjectCorrectionAcknowledgement(bodyText)} Contame en qué más te ayudo.`;
  const wrapped = buildAutoReplyWithGreetingIfNeeded(acknowledgement, profileDisplayName, mergedState);
  await setConversationState(
    from,
    mergeConversationStatePreservingGreeting(mergedState, {}, {
      ...(wrapped.nextStatePatch || {}),
...
async function buildCoupleConsultationGuidanceReplies(priorState, rawText, lastSede, healthInsuranceName) {
  const normalized = normalizeForMatch(rawText);
  const replies = [];
  const sameDay = normalized.includes('mismo dia') || normalized.includes('mismo día');
  const mixedCoverage = messageMentionsMixedCoupleCoverage(rawText);
  replies.push(
    'Para consultar los dos necesitan un turno por persona: una evaluación cada uno. Por este chat no reservo turnos ni confirmo horarios.'
  );
  if (mixedCoverage && healthInsuranceName) {
    replies.push(
      `Vos con ${healthInsuranceName} y tu esposo/a particular: cada uno abona según su cobertura en su consulta.`
    );
  } else if (healthInsuranceName && lastSede) {
    replies.push(await buildHealthInsuranceCoverageLineForSede(lastSede, healthInsuranceName));
    const privatePriceReply = await buildPrivatePriceReply(lastSede);
    if (privatePriceReply) {
      replies.push(`Como referencia particular en ${lastSede.displayName}: ${privatePriceReply}`);
    }
  } else if (lastSede) {
    const privatePriceReply = await buildPrivatePriceReply(lastSede);
    if (privatePriceReply) {
      replies.push(`Como referencia en ${lastSede.displayName}: ${privatePriceReply} por persona.`);
    }
  }
  if (sameDay && lastSede && !isReferralOnlySedeEntry(lastSede)) {
    replies.push(
      'Si buscan el mismo día, en la agenda online podés mirar si hay dos horarios libres; no puedo confirmar disponibilidad por acá.'
    );
    replies.push(buildMicroCommitmentMessage());
  } else if (sameDay && lastSede && isReferralOnlySedeEntry(lastSede)) {
    replies.push(buildReferralOnlySedeBookingReply(lastSede));
  } else if (lastSede && !isReferralOnlySedeEntry(lastSede)) {
    replies.push(buildMicroCommitmentMessage());
  }
  return replies;
}

async function tryHandleCoupleConsultationInquiry(from, bodyText, priorState, profileDisplayName) {
  if (!messageLooksLikeCoupleConsultationInquiry(bodyText)) return false;
  const patientContext = await resolvePatientContextFromMessage(bodyText, priorState, { profileDisplayName });
  const mergedState = mergeConversationStatePreservingGreeting(
    priorState,
    priorState || {},
    {
      ...(patientContext.statePatch || {}),
      ...buildPatientSubjectStatePatch('spouse'),
    }
  );
  const lastSede = patientContext.sedeEntry || resolveLastSedeEntryFromState(mergedState);
  if (!lastSede) return false;
  ...
  const replies = await buildCoupleConsultationGuidanceReplies(
    mergedState,
    bodyText,
    lastSede,
    healthInsuranceName
  ...
  const statePatch = {
    ...
    ...(healthInsuranceName
      ...
      : {}),
    ...(!isReferralOnlySedeEntry(lastSede)
      ? buildAwaitingLinkConfirmationState(lastSede, 'after_couple_consultation_guidance', {
          healthInsuranceName: healthInsuranceName || undefined,
        })
      : {}),
  };
  return deliverSequentialPatientTextMessages(from, replies, priorState, profileDisplayName, statePatch, {
    userMessage: bodyText,
    replyContext: 'couple_consultation_guidance',
    suppressBookingLinkOffer: isReferralOnlySedeEntry(lastSede),
  });
}
...
function messageShouldSkipOpenAiAssistantFallback(rawText, priorState) {
  ...
  if (messageAsksToChangeBookingSede(rawText)) return true;
  if (messageLooksLikeSpirometrySchedulingInquiry(rawText)) return true;
...
function messageLooksLikeFamilyConsultationCostEstimateInquiry(rawText) {
  ...
  if (messageLooksLikeCoupleConsultationInquiry(rawText)) return false;
  return (
    messageAsksApproximateConsultationCost(rawText) ||
    (messageLooksLikeAnyPriceQuestion(rawText) &&
...
async function estimatePrivateConsultationAndStandaloneSpirometryTotal(sedeEntry) {
  const consultationAmountArs = await lookupPrivatePrice(sedeEntry.displayName);
  if (!Number.isFinite(consultationAmountArs)) return null;
  const studyAmountArs = STANDALONE_SPIROMETRY_PRICE_ARS;
  return {
    consultationAmountArs,
    studyAmountArs,
    totalAmountArs: consultationAmountArs + studyAmountArs,
    isPrivatePay: true,
    studyType: 'espirometría',
    usesStandaloneSpirometryPrice: true,
  };
}

function priorStateRecentlyOfferedPrivateTotal(priorState) {
  if (!priorState || typeof priorState !== 'object') return false;
  const lastBotReplyText =
    typeof priorState.lastBotReplyText === 'string' ? priorState.lastBotReplyText.trim() : '';
  if (!lastBotReplyText) return false;
  const lastBotReplyAtMs = Number(priorState.lastBotReplyAtMs);
  if (!Number.isFinite(lastBotReplyAtMs) || Date.now() - lastBotReplyAtMs > PRICE_OBJECTION_CONTEXT_WINDOW_MS) {
    return false;
  }
  const normalized = normalizeForMatch(lastBotReplyText);
  return (
    normalized.includes('no trabajamos') ||
    normalized.includes('hacerlo particular') ||
    normalized.includes('te paso el total') ||
    normalized.includes('decime y te paso')
  );
}

function priorStateRecentlyQuotedStandaloneSpirometryPrice(priorState) {
  if (!priorState || typeof priorState !== 'object') return false;
  const lastBotReplyText =
    typeof priorState.lastBotReplyText === 'string' ? priorState.lastBotReplyText.trim() : '';
  if (!lastBotReplyText) return false;
  const normalized = normalizeForMatch(lastBotReplyText);
  return normalized.includes('solo por espirometr') || normalized.includes('venis solo por espirometr');
}

function messageAsksConsultationInAdditionToStudy(rawText, priorState = null) {
  if (!rawText || typeof rawText !== 'string') return false;
  const normalized = normalizeForMatch(rawText)
    .replace(/[!?.,;:]+/g, ' ')
    .replace(/\s+/g, ' ')
  ...
  if (!normalized.includes('consulta')) return false;
  const asksAlso =
    normalized.includes('ademas') ||
    normalized.includes('además') ||
  ...
  const hasSpirometryContext =
    normalized.includes('espirometr') ||
    priorStateIndicatesSpirometryStudy(priorState) ||
    priorStateRecentlyQuotedStandaloneSpirometryPrice(priorState);
  if (!hasSpirometryContext) return false;
  return asksAlso || messageAsksCompleteOrTotalCost(rawText) || messageLooksLikeAnyPriceQuestion(rawText);
}

async function buildPrivateConsultationAndStandaloneSpirometryTotalReply(sedeEntry) {
  const estimate = await estimatePrivateConsultationAndStandaloneSpirometryTotal(sedeEntry);
  if (!estimate) return null;
  const consultationFormatted = formatArsAmount(estimate.consultationAmountArs);
  const studyFormatted = formatArsAmount(estimate.studyAmountArs);
  const totalFormatted = formatArsAmount(estimate.totalAmountArs);
  return `En ${sedeEntry.displayName}, consulta particular $${consultationFormatted} + espirometría $${studyFormatted}. Total aproximado: $${totalFormatted}.`;
}
  ...
  if (healthInsuranceName) {
    ...
    if (!estimate) {
      ...
      if (privateEstimate) {
        ...
        const studyFormatted = formatArsAmount(privateEstimate.studyAmountArs);
        const totalFormatted = formatArsAmount(privateEstimate.totalAmountArs);
        return `${warmPrefix}Con ${healthInsuranceName} en ${cityName} no trabajamos. En particular: consulta $${consultationFormatted} + ${studyWithArticle} $${studyFormatted}. Total aproximado: $${totalFormatted}.`;
      }
      return `${warmPrefix}Con ${healthInsuranceName} en ${cityName} no trabajamos. Si querés el total particular, escribime "particular" y te lo detallo.`;
    }
...
async function tryHandleCompleteCostTotalInquiry(from, bodyText, priorState, profileDisplayName) {
  ...
  const lastSede = patientContext.sedeEntry || resolveLastSedeEntryFromState(mergedState);
  let isPrivatePay = await resolvePrivatePayWithoutHealthInsuranceFromMessage(bodyText, {
    priorState: mergedState,
    profileDisplayName,
  });
  if (
    !isPrivatePay &&
    priorStateRecentlyOfferedPrivateTotal(mergedState) &&
    (messageAsksCompleteOrTotalCost(bodyText) || messageStatesPrivatePayWithoutHealthInsurance(bodyText))
  ) {
    isPrivatePay = true;
  }
  ...
  if (
    messageLooksLikeRealtimeAvailabilityQuestion(bodyText) ||
    messageLooksLikeBookingIntent(bodyText) ||
    messageAsksAboutAfternoonAvailability(bodyText)
  ) {
    if (!isReferralOnlySedeEntry(lastSede) && getAgendaUrl(lastSede)) {
      replies.push(
        `Para ver si hay turno el día y horario que buscás (por ejemplo después de las 17), elegí en la agenda online. ${buildMicroCommitmentMessage()}`
      );
    }
  }
  if (replies.length === 1) {
  ...
  const statePatch = {
    ...
    ...buildLastHealthInsuranceDiscussionStatePatch(),
    ...(!isReferralOnlySedeEntry(lastSede)
      ? buildAwaitingLinkConfirmationState(lastSede, 'after_complete_cost_total', {
          healthInsuranceName: healthInsuranceName || undefined,
        })
      : {}),
  };
  return deliverSequentialPatientTextMessages(from, replies, mergedState, profileDisplayName, statePatch, {
    userMessage: bodyText,
    replyContext: 'complete_cost_total',
  });
}

async function tryHandleConsultationPlusStandaloneSpirometryTotalInquiry(
  from,
  bodyText,
  priorState,
  profileDisplayName
) {
  if (!messageAsksConsultationInAdditionToStudy(bodyText, priorState)) return false;
  const patientContext = await resolvePatientContextFromMessage(bodyText, priorState, { profileDisplayName });
  const mergedState = mergeConversationStatePreservingGreeting(
    priorState,
    priorState || {},
  ...
  const lastSede = patientContext.sedeEntry || resolveLastSedeEntryFromState(mergedState);
  if (!lastSede) return false;
  const reply = await buildPrivateConsultationAndStandaloneSpirometryTotalReply(lastSede);
  if (!reply) return false;
  return sendFinalizedPatientTextReply(
    from,
    reply,
...
async function tryHandlePrivatePayTotalFollowUpInquiry(from, bodyText, priorState, profileDisplayName) {
  if (!priorStateRecentlyOfferedPrivateTotal(priorState)) return false;
  const wantsPrivateTotal =
    messageStatesPrivatePayWithoutHealthInsurance(bodyText) ||
    (await resolvePrivatePayWithoutHealthInsuranceFromMessage(bodyText, {
      priorState,
      profileDisplayName,
    }));
  if (!wantsPrivateTotal) return false;
  ...
  let reply = null;
  if (
    priorStateRecentlyQuotedStandaloneSpirometryPrice(mergedState) &&
    priorStateIndicatesSpirometryStudy(mergedState)
  ) {
    reply = await buildPrivateConsultationAndStandaloneSpirometryTotalReply(lastSede);
  }
  if (!reply) {
    reply = await buildCompleteCostTotalReply(lastSede, null, studyType, bodyText);
  }
  if (!reply) return false;
...
async function tryHandlePatientTotalCostInquiries(from, bodyText, priorState, profileDisplayName) {
  if (await tryHandlePrivatePayTotalFollowUpInquiry(from, bodyText, priorState, profileDisplayName)) {
    return true;
  }
  if (
    await tryHandleConsultationPlusStandaloneSpirometryTotalInquiry(
      from,
      bodyText,
      priorState,
      profileDisplayName
...
function messageShouldBypassSedeCityDeclaration(rawText) {
  ...
  if (messageLooksLikeRichPatientIntakeInquiry(rawText)) return true;
  if (messageLooksLikeComplexClinicalInquiry(rawText)) return true;
  if (messageLooksLikeCompleteCostTotalInquiry(rawText)) return true;
  if (messageLooksLikePatientTopicsOrchestratorCandidate(rawText)) return true;
  if (messageLooksLikeCombinedConsultationAndStudyPriceInquiry(rawText)) return true;
  if (messageLooksLikeMultiIntentCandidate(rawText)) return true;
  return false;
...
async function buildStudiesInformationReply(priorState, rawText = '', options = {}) {
  ...
  const studyTypeFromMessage = await resolveStudyTypeFromMessage(rawText, priorState, {
    profileDisplayName:
      options && typeof options.profileDisplayName === 'string' ? options.profileDisplayName : '',
  });
  const studyTypeFromState =
  ...
  if (
    messageLooksLikeSpirometryOnlyInquiry(rawText) ||
    (isSpirometryStudyType(studyTypeFromMessage) &&
      messageLooksLikeSpirometryPriceOnlyInquiry(rawText, studyTypeFromMessage))
  ) {
  ...
  if (isPatchStudyType(studyType) && asksPrice && !messageAsksCompleteOrTotalCost(rawText)) {
    return buildAllergyTestRequiresEvaluationPriceReply(lastSede);
  }
...
function buildAlreadySentBookingLinkAffirmationReply(priorState, entry, options = {}) {
  const patientConfirmedSede = resolvePatientConfirmedSedeEntryFromState(priorState);
  if (patientConfirmedSede && isReferralOnlySedeEntry(patientConfirmedSede)) {
    return buildReferralOnlySedeBookingReply(patientConfirmedSede);
  }
  if (isReferralOnlySedeEntry(entry)) {
    return buildReferralOnlySedeBookingReply(entry);
  }
...
function messageAsksToChangeBookingSede(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  if (!messageMentionsExistingBookingContext(rawText)) return false;
  const normalized = normalizeForMatch(rawText)
    .replace(/[!?.,;:]+/g, ' ')
    ...
    .trim();
  if (!normalized) return false;
  const sedesInOrder = findAllSedeEntriesFromTextInOrder(rawText);
  const wantsDifferentSede =
    normalized.includes('quiero atenderme en') ||
    normalized.includes('quiero atenderme') ||
    normalized.includes('quiero ir a') ||
    normalized.includes('quiero ir en') ||
    normalized.includes('prefiero atenderme') ||
    normalized.includes('prefiero ir') ||
    normalized.includes('mejor en') ||
    normalized.includes('ahora en') ||
    normalized.includes('pasarme a') ||
    normalized.includes('pasar a') ||
    normalized.includes('cambiar de sede') ||
    normalized.includes('cambiar la sede') ||
    normalized.includes('cambiar sede') ||
    normalized.includes('otra sede') ||
    normalized.includes('en otra ciudad') ||
    normalized.includes('en otra sede');
  if (
    sedesInOrder.length >= 2 &&
    (normalized.includes('pero') || normalized.includes('ahora') || wantsDifferentSede)
  ) {
    return true;
  }
  if (wantsDifferentSede && sedesInOrder.length >= 1) return true;
  return false;
}

function resolveExistingBookingSedeForChangeRequest(rawText, priorState) {
  const sedesInOrder = findAllSedeEntriesFromTextInOrder(rawText);
  if (sedesInOrder.length >= 2) return sedesInOrder[0];
  const originFromState =
    resolveLastSedeEntryFromState(priorState) || resolveSedeEntryFromState(priorState);
  if (
    sedesInOrder.length === 1 &&
    originFromState &&
    sedesInOrder[0].displayName !== originFromState.displayName
  ) {
    return originFromState;
  }
  return originFromState || sedesInOrder[0] || null;
}

function resolveTargetSedeForBookingChangeRequest(rawText, priorState) {
  const sedesInOrder = findAllSedeEntriesFromTextInOrder(rawText);
  if (sedesInOrder.length >= 2) return sedesInOrder[sedesInOrder.length - 1];
  const bookingSede = resolveExistingBookingSedeForChangeRequest(rawText, priorState);
  if (
    sedesInOrder.length === 1 &&
    bookingSede &&
    sedesInOrder[0].displayName !== bookingSede.displayName
  ) {
    return sedesInOrder[0];
  }
  return sedesInOrder[0] || null;
}

function buildChangeBookingSedeReply(priorState, rawText) {
  const bookingSede = resolveExistingBookingSedeForChangeRequest(rawText, priorState);
  const targetSede = resolveTargetSedeForBookingChangeRequest(rawText, priorState);
  const intro =
    'Sin problema 😊 Si ya tenés turno y querés cambiar de sede, eso lo coordina nuestro equipo por teléfono. Por este chat no movemos reservas ni generamos un turno nuevo.';
  const closing = 'Ellos tienen acceso a la agenda y te ayudan con el cambio.';
  if (bookingSede) {
    const phoneNumber = resolveClinicAssistancePhoneNumberForSedeEntry(bookingSede);
    const targetLabel =
...
async function tryHandleChangeBookingSedeInquiry(
  ...
  if (!messageAsksToChangeBookingSede(bodyText)) return false;
  const reply = buildChangeBookingSedeReply(priorState, bodyText);
  const finalizedReply = await finalizePatientReplyText(reply, {
    priorState,
    profileDisplayName,
    userMessage: bodyText,
    replyContext: 'change_booking_sede',
    suppressBookingLinkOffer: true,
    skipHumanization: true,
    conversationContext: buildIntentRoutingOpenAiContext(priorState),
  });
  const wrapped = buildAutoReplyWithGreetingIfNeeded(finalizedReply, profileDisplayName, priorState);
  const bookingSede = resolveExistingBookingSedeForChangeRequest(bodyText, priorState);
  await setConversationState(
    from,
    mergeConversationStatePreservingGreeting(priorState, priorState || {}, {
      ...(wrapped.nextStatePatch || {}),
      ...(bookingSede ? buildLastSedeStatePatch(bookingSede) || {} : {}),
      ...buildLastBotReplyStatePatch(wrapped.messageText),
      bookingLinkOptOutUntilMs: Date.now() + BOOKING_LINK_OFFER_OPTOUT_MS,
      lastBotReplyAtMs: Date.now(),
      lastSeenAtMs: Date.now(),
...
function messageAsksToRescheduleOrCancelBooking(rawText) {
  ...
  const hasExistingBookingContext =
    normalized.includes('tengo turno') ||
    normalized.includes('ya tengo turno') ||
    normalized.includes('mi turno') ||
    normalized.includes('turno el') ||
    normalized.includes('turno para');
  const wantsChange =
    normalized.includes('cambiar el turno') ||
    normalized.includes('cambiar turno') ||
    normalized.includes('cambiarlo') ||
    normalized.includes('cambiarla') ||
    normalized.includes('para otro dia') ||
    normalized.includes('para otro día') ||
    normalized.includes('otro dia') ||
    normalized.includes('otro día') ||
  ...
  if (wantsChange && hasExistingBookingContext) return true;
  return (
    normalized.includes('cambiar el turno') ||
    normalized.includes('cambiar turno') ||
    ...
    normalized.includes('re-program') ||
    normalized.includes('posponer') ||
    normalized.includes('cancelar turno') ||
    normalized.includes('cancelar mi turno') ||
    normalized.includes('anular turno') ||
    normalized.includes('anular mi turno')
  );
}

function buildRescheduleOrCancelBookingReply(priorState, rawText) {
  const lastSede =
    findSedeFromText(rawText) ||
    resolveLastSedeEntryFromState(priorState) ||
    resolveSedeEntryFromState(priorState);
  const intro =
    'Sin problema 😊\nLos cambios y reprogramaciones de turnos los gestiona directamente nuestro equipo.';
...
async function tryHandleRescheduleOrCancelBookingInquiry(
  ...
  if (!messageAsksToRescheduleOrCancelBooking(bodyText)) return false;
  const reply = buildRescheduleOrCancelBookingReply(priorState, bodyText);
  const finalizedReply = await finalizePatientReplyText(reply, {
    priorState,
    profileDisplayName,
    userMessage: bodyText,
    replyContext: 'reschedule_booking',
    suppressBookingLinkOffer: true,
    conversationContext: buildIntentRoutingOpenAiContext(priorState),
  });
  const wrapped = buildAutoReplyWithGreetingIfNeeded(finalizedReply, profileDisplayName, priorState);
...
function messageSaysDoesNotKnowHealthInsurance(rawText) {
  if (!rawText || typeof rawText !== 'string') return false;
  if (messageAsksToConfirmExistingBooking(rawText)) return false;
  if (messageAsksAboutExistingBookingScheduleRecall(rawText)) return false;
  const normalized = normalizeForMatch(rawText)
    .replace(/[!?.,;:]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  if (!normalized) return false;
  const mentionsHealthInsuranceContext =
    normalized.includes('obra social') ||
    normalized.includes('prepaga') ||
    normalized.includes('cobertura') ||
    ...
    messageLooksLikeHealthInsurancePlusQuestion(rawText);
  if (
    (normalized.includes('no me acuerdo') || normalized.includes('no recuerdo')) &&
    !mentionsHealthInsuranceContext &&
    (normalized.includes('turno') || normalized.includes('reserva') || normalized.includes('cita') || extractWeekdayNameFromText(rawText))
  ) {
    return false;
  }
  return (
    ...
    (normalized.includes('no me acuerdo') && mentionsHealthInsuranceContext) ||
    (normalized.includes('no recuerdo') && mentionsHealthInsuranceContext) ||
    ...
    for (const item of latestMessageBySender.values()) {
      ...
      if (regionResetState !== priorState) {
        ...
        await setConversationState(from, {
          ...
          if (
            await tryHandleChangeBookingSedeInquiry(from, bodyText, priorState, profileDisplayName, {
              inboundAtMs,
              updatedRecentIds,
            })
          ) {
          ...
          if (
            await tryHandleRescheduleOrCancelBookingInquiry(from, bodyText, priorState, profileDisplayName, {
              inboundAtMs,
              updatedRecentIds,
            })
          ) {
          ...
          if (
            await tryHandleSubstantivePatientInquiryBeforeSedeRouting(
              from,
              bodyText,
              priorState,
              profileDisplayName
          ...
          if (messageAsksHowBookingWorks(bodyText)) {
            ...
            if (confirmedSede) {
              if (isReferralOnlySedeEntry(confirmedSede)) {
                await sendReferralOnlySedeBookingReply(from, confirmedSede, priorState, profileDisplayName, bodyText);
                continue;
              }
            ...
            const referralSede = resolveReferralSedeForConversationFollowUp(priorState, bodyText);
            if (referralSede) {
              await sendReferralOnlySedeBookingReply(from, referralSede, priorState, profileDisplayName, bodyText);
              continue;
            }
            const mergedState = mergeConversationStatePreservingGreeting(
              priorState,
          ...
          if (
            messageLooksLikePatientTopicsOrchestratorCandidate(bodyText, priorState) &&
            collectPatientReplyTopicsFromMessage(bodyText, priorState).length >= 2 &&
            !messageAsksStudyProcedurePrice(bodyText) &&
          ...
          const resolvedStudyTypeForGate = await resolveStudyTypeFromMessage(bodyText, priorState, {
            profileDisplayName,
          });
          if (
            ...
            (messageAsksAboutStudyPrice(bodyText) ||
              ...
              Boolean(resolvedStudyTypeForGate) ||
              messageLooksLikeVagueOrMisspelledStudyMention(bodyText) ||
            ...
            const detectedStudyType =
              resolvedStudyTypeForGate ||
              (await resolveStudyTypeFromMessage(bodyText, priorState, { profileDisplayName }));
          ...
          if (stateLooksLikeAwaitingLinkConfirmation(priorState)) {
            ...
            const shouldBypassPendingLinkConfirmation =
              ...
              (messageLooksLikeMultiQuestionPatientInquiry(bodyText, priorState) ||
                ...
                messageLooksLikeChronicSymptomFrustration(bodyText));
            if (shouldBypassPendingLinkConfirmation) {
              if (messageLooksLikeMultiQuestionPatientInquiry(bodyText, priorState)) {
                if (await tryHandleMultiQuestionPatientInquiryWithOpenAi(from, bodyText, priorState, profileDisplayName)) {
                  continue;
                }
              }
